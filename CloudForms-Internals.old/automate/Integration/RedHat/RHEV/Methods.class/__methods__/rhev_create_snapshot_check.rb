=begin
 rhev_create_snapshot_check.rb

 Author: Kevin Morey <kevin@redhat.com>

 Description: This is method checks that snapshots identified in $evm.get_state_var(:created_snapshots)
...
begin
  ...
  created_snapshots = $evm.get_state_var(:created_snapshots)
  log(:info, "{:created_snapshots=>#{created_snapshots}}")

  created_snapshots.each do |snapshot_ref|
    ...
    snapshot_status = check_status_response_hash["snapshot_status"]

    if snapshot_status == 'ok'
      log(:info, "Successfully created snapshot: #{snapshot_description} snapshot_status: #{snapshot_status}", true)
      @vm.custom_set(:SNAPSHOT_CREATE_STATUS, nil)
    else
      @vm.custom_set(:SNAPSHOT_CREATE_STATUS, snapshot_status)
      retry_method('30.seconds', "snapshot_description: #{snapshot_description} snapshot_status: #{snapshot_status}")
    end
  end
