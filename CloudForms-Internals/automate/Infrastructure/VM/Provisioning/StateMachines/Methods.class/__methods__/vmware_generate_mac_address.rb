=begin
 ...
 Description: This method generates a VMware based mac address
-------------------------------------------------------------------------------
...
begin
  ...
  when 'miq_provision'
    ...
    log(:info, "Provision: #{@task.id} Request: #{@task.miq_provision_request.id} provision_type: #{@task.get_option(:provision_type)}")
  ...
  when 'vmware'
    ...
    # log(:info, "Provisioning object updated {:mac_address => #{@task.get_option(:mac_address)}}", true)
  when 'pxe'
    macaddress = generate_unique_macaddress(nic_prefix)
    #@task.set_network_adapter(0, {:mac_address => macaddress})
    @task.set_option(:mac_address, macaddress)
    ...
    log(:info, "provision_type: #{@task.get_option(:provision_type)} does not match, skipping method...")
  end
