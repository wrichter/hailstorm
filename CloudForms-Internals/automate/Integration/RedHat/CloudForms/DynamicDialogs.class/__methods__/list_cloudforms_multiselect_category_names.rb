=begin
 list_cloudforms_multiselect_category_names.rb

 Author: Kevin Morey <kevin@redhat.com>

 Description: This method lists CloudForms multi-select Tag Category Names
-------------------------------------------------------------------------------
...
$evm.vmdb('classification').all.each do |cat|
  ...
  next if cat.single_value
    dialog_hash[cat.name] = "#{cat.name} - #{cat.description}"
end

if dialog_hash.blank?
  dialog_hash[''] = "< No categories found >"
else
